...
trait EventEmitterTrait {
    ...
    public function on($event, callable $listener) {
        if ( $event === null ) throw new InvalidArgumentException('event name must not be null');
    ...
    public function once($event, callable $listener) {
        if ( $event === null ) throw new InvalidArgumentException('event name must not be null');
        if ( !isset($this->onceListeners[$event]) ) $this->onceListeners[$event] = [];
    ...
    public function removeListener($event, callable $listener) {
        ...
        if ( isset($this->onceListeners[$event]) ) {
            ...
            if ( false !== $index ) {
                ...
                if ( \count($this->onceListeners[$event]) === 0 ) unset($this->onceListeners[$event]);
            }
        }
    }
